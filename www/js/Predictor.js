var Predictor = {
    runPrediction : function () {
        var predictionData = $('#predict-form').serializeForm(), myFlo = globalFlos[predictionData.predictItem], myPrediction = Flos.getStatus(predictionData.predictDate, myFlo.startDate, myFlo.cycle, myFlo.long);
    ...
    reportPrediction : function (predictionData, myFlo, myPrediction) {
        ...
        var newDateFormat = moment(predictionData.predictDate).format('MMMM Do, YYYY'), bIsFuture = makeSureFuture(predictionData.predictDate);
        if (!bIsFuture) {
...
$("#predict-form #predictDate").on('blur', function () {
    var dateValue = this.value, bIsFuture = makeSureFuture(dateValue);
    if (!bIsFuture) {
        Predictor.getNextDay();
    }
